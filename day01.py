...
def get_calibration_value(line: str) -> int:
    ...
    for ch in line:
        if not ch.isdigit():
            continue
        # if less than 2 elements, store it
        # if not empty, replace the second number
        if len(chars) < 2:
            chars.append(ch)
        else:
            chars[1] = ch
...
def day1():
    ...
    return functools.reduce(lambda s, v: s + v, values)
...
def day2():
    new_lines = [to_nums(l) for l in lines]
    values = [get_calibration_value(l) for l in new_lines]
    return functools.reduce(lambda s, v: s + v, values)
