...
def line_to_results(line: str):
    ...
    for result in games:
        by_color = {}
        cube_results = result.split(",")
        for cubes in cube_results:
            count, color = cubes.strip().split(" ")
            by_color[color.strip()] = int(count)
        results_arr.append(by_color)
    return (game_id, results_arr)

def is_result_valid(result: dict[str, int]):
    for color in result:
        if available_colors[color] < result[color]:
            return False
    return True

def is_game_valid(game: tuple[str, list]):
...
def find_max_cubes(game: list[dict[str, int]]):
    max_cubes = {}
    for result in game:
        for color in result:
            count = result[color]
            max_cubes[color] = max(max_cubes.get(color, 0), count)
    return max_cubes
...
def power(cubes: dict[str, int]):
    return cubes.get("red", 0) * cubes.get("blue", 0) * cubes.get("green", 0)


def day1():
    lines = get_lines(input_str)
    results = map(line_to_results, lines)
    valid_games = [int(game_id) for game_id, r in results if is_game_valid((game_id, r))]
    return functools.reduce(add, valid_games)

def day2():
    games = [line_to_results(line) for line in get_lines(input_str)]
    max_cubes = [power(find_max_cubes(g)) for g_id, g in games]
    return functools.reduce(add, max_cubes)
