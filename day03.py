...
def line_to_coords(line_idx: int, line: str):
    curr_num = []
    curr_range = []
    nums = []
    symbols = []
    for idx in range(0, len(line)):
        ...
        if (char.isnumeric()):
            ...
            curr_range.append(idx)
        else:
            if (not char == "."):
                symbols.append((char, (line_idx, idx)))

            if (len(curr_num) > 0):
                num = int("".join(curr_num))
                coords = [(line_idx, x) for x in curr_range]
...
def build_coords_dict(nums_and_symbols: tuple[list, dict[str, list]], coords: tuple[list, list]):
    nums, symbols = nums_and_symbols
    cnum, csym = coords
    ...
    for s in csym:
        symbol, s_coords = s
        symbols[s_coords] = symbol
    
    return (nums, symbols)

def neighbors(coord: tuple[int, int]):
    y, x = coord
    return [
        (y+1, x),
        (y-1, x),
...
def is_number_adjacent(num: list[tuple[int, int]], symbol_map: dict[tuple[int, int], str]):
    for coord in num:
        for neighbor in neighbors(coord):
            if (symbol_map.get(neighbor) != None):
                return True
    return False


def day1():
    coords = [line_to_coords(idx, lines[idx]) for idx in range(0, len(lines))]
    nums, symbols = functools.reduce(build_coords_dict, coords, ([], {}))
    part_nums = [num for (num, coords) in nums if is_number_adjacent(coords, symbols)]
    return functools.reduce(util.add, part_nums)


def all_coords_to_map(coord_map: tuple[dict, list], coords_list: tuple[list, list]):
    grid, gears = coord_map
    nums, symbols = coords_list
...
def neighboring_gear_values(gear_coords: tuple[int, int], coords_map: dict):
    return list(set([coords_map.get(n) for n in neighbors(gear_coords) if isinstance(coords_map.get(n), int)]))

def is_gear(gear_coords: tuple[int, int], coords_map: dict):
    neighboring_values = list(set([coords_map.get(n) for n in neighbors(gear_coords) if isinstance(coords_map.get(n), int)]))
    return len(neighboring_values) == 2

def day2():
    coords = [line_to_coords(idx, lines[idx]) for idx in range(0, len(lines))]
    grid, gears = functools.reduce(all_coords_to_map, coords, ({}, []))
    valid_gears = [neighboring_gear_values(g, grid) for g in gears if len(neighboring_gear_values(g, grid)) == 2]
    gear_ratios = [a * b for [a, b] in valid_gears]
    return functools.reduce(util.add, gear_ratios)
