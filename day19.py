...
class Rule:
    ...
    def __init__(self, line: str) -> None:
        ...
        if len(parts) > 1:
            ...
            self.workflow = parts[0]
    def __repr__(self) -> str:
        if self.condition != None:
            return self.condition + ":" + self.workflow
        else:
            return self.workflow
    def matches_condition(self, part: dict[str, int]):
        match self.condition:
            case None:
                return True
            case c if "<" in c:
                rating, value = c.split("<")
                return part[rating] < int(value)
            case c if ">" in c:
                rating, value = c.split(">")
                return part[rating] > int(value)
            case _:
                raise ValueError("Invalid condition:" + self.condition)
...
class Workflow:
    ...
    def __repr__(self) -> str:
        return (self.name, self.rules).__repr__()
    def process(self, part: dict[str, int]) -> str:
        # return next workflow
        for rule in self.rules:
            if (rule.matches_condition(part)):
                return rule.workflow
        return None


def process_lines(processed: tuple[dict[str, Workflow], list[dict[str, int]]], line: str):
    workflows, parts = processed
    if line.startswith("{"):
        r_str = line.replace("{", "").replace("}", "").split(",")
        ratings = {}
        for rating in r_str:
            r, val = rating.split("=")
            ratings[r] = int(val)
...
def process_part(part: dict[str, int], workflows: dict[str, Workflow]):
    wf = workflows["in"]
    while True:
        next_wf = wf.process(part)
        if next_wf == "A":
            return "A"
        elif next_wf == "R":
            return "R"
...
def part_value(part: dict[str, int]):
    return functools.reduce(util.add, part.values())


def part1():
    lines = util.get_lines(input_str)
    workflows, parts = functools.reduce(process_lines, lines, ({}, []))
