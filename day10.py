...
def get_connected_tiles(pos: tuple[int, int], tile: str):
    y, x = pos
    match tile:
        case "|":
            return [(y - 1, x), (y + 1, x)]
        ...
        case ".":
            return []
        case "S":
            # need to find the two tiles that connect to this one
            return []
...
def parse_map(lines: list[str]):
    ...
    for (y, line) in enumerate(lines):
        for (x, tile) in enumerate(line):
            connected = get_connected_tiles((y, x), tile)
            coords[(y, x)] = connected
            if tile == "S":
                start = (y, x)
    coords[start] = [c for c in coords if start in coords[c]]
    return (start, coords)

def find_longest_path(start: tuple[int, int], coords: dict[tuple[int, int], list[tuple[int, int]]]):
    max_val = 9e99
    queue = [(start, 0)]
    visited = {}
    max_path = (0, None)
    ...
    while len(queue) > 0:
        ...
        if path > max_path[0]:
            max_path = (path, head)
        visited[head] = min(path, visited.get(head, max_val))
        next_tiles = [(pos, visited[head] + 1) for pos in coords[head] if (not pos in visited)]
...
def debug(lines: list[list[str]], path: dict):
    output = []
    for y, line in enumerate(lines):
        o = []
        for x, tile in enumerate(line):
...
def part1():
    (start, coords) = parse_map(lines)
    (longest, _) = find_longest_path(start, coords)
    return longest
...
def part2():
    # lines = util.get_lines(example_input3)
    (start, coords) = parse_map(lines)
    (longest, path) = find_longest_path(start, coords)
    print(debug(lines, path))
    print("Path", len(path))
