...
def parse_input(lines: list[str]):
    ...
    for i in range(1, len(lines)):
        ...
        nodes[node] = {"L": left.replace("(", ""), "R": right.replace(")", "")}
...
def part1():
    ...
    moves = 0
    while cur_node != "ZZZ":
        direction = directions[moves % len(directions)]
...
def part2():
    ...
    for n in cur_nodes:
        ...
        while (not cur_node.endswith("Z")):
            direction = directions[mcount % len(directions)]
            cur_node = nodes[cur_node][direction]
            mcount = mcount + 1
        counts.append(mcount)
    return functools.reduce(lambda prod, count: compute_lcm(prod,count), counts)
